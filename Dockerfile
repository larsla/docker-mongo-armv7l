...
RUN set -ex && apk add --no-cache bash git curl tar scons g++ linux-headers openssl-dev perl && \
    ...
    bash gen-config.sh arm linux && cd - && \
    ...
    cd / && apk del bash git curl tar scons g++ linux-headers openssl-dev perl && \
    rm -rf /etc/ssl /tmp/* \
